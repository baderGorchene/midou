...
const normalizeRole = (r, roles) => {
  ...
  if (typeof r === "string") {
    const found = roles.find(
      (role) => role.label?.toLowerCase() === r.trim().toLowerCase(),
    );
    return found?.value ?? roles[0]?.value ?? 1;
  }

  return roles[0]?.value ?? 1;
};
...
const EditModal = ({
  ...
  React.useEffect(() => {
    if (user) {
      ...
      setSelectedRole(normalizeRole(user.role ?? user.Role, roles));
      ...
      >
        ...
        <View style={s.sheet}>
          ...
          <ScrollView showsVerticalScrollIndicator={false}>
            ...
            <Text style={s.label}>Département</Text>
...
const UserManagementScreen = () => {
  ...
  const loadLookups = useCallback(async () => {
    ...
    const normalizedDepartments = normalizeBackendItems(extractArray(depsRes));
  ...
  const loadUsers = useCallback(
    async (opts = {}) => {
      ...
      try {
        ...
        const roleLabel = roleVal
          ? rolesList.find((r) => r.value === roleVal)?.label
          : undefined;
  ...
  useFocusEffect(
    useCallback(() => {
      ...
      const loadAll = async () => {
        try {
          setLoading(true);
          const loadedRoles = await loadLookups();

          if (!isActive) return;

          await loadUsers({
            rolesList: loadedRoles,
            searchVal: "",
            roleVal: null,
          });
        } catch (error) {
  ...
  const handleRoleChange = async (user, newRole) => {
    ...
    try {
      ...
      setUsers((prev) =>
        prev.map((u) =>
          (u.id ?? u.Id) === id ? { ...u, role: newRole, Role: newRole } : u,
        ),
      );
  ...
  const renderUserCard = (user) => {
    ...
    const role = normalizeRole(user.role ?? user.Role, roles);
    ...
    return (
      <View key={id} style={styles.card}>
        <View style={styles.cardHeader}>
          <View
            style={[styles.avatar, { backgroundColor: `${roleMeta.color}18` }]}
          >
            ...
            {roles.map((opt) => {
              ...
              return (
                <TouchableOpacity
                  ...
                  onPress={() =>
                    !isChangingRole && handleRoleChange(user, opt.value)
                  }
                ...
                >
                  {isChangingRole && sel ? (
                    ...
                    <Ionicons
                      name={opt.icon}
                      size={12}
                      color={sel ? opt.color : colors.textSecondary}
                    />
                  )}
  ...
  return (
    <View style={styles.container}>
        ...
        {roles.map((r) => (
          ...
          >
            <Ionicons
              name={r.icon}
              size={12}
              color={roleFilter === r.value ? r.color : colors.textSecondary}
            />
            <Text
              style={[
      ...
      <View style={styles.statsRow}>
        ...
        <View style={styles.statItem}>
          <Text style={styles.statNum}>{roles.length}</Text>
          <Text style={styles.statLabel}>Rôles</Text>
        </View>
      </View>
